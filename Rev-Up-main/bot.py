...
API_HASH = "abcdef1234567890abcdef1234567890"  # Reemplaza con tu API_HASH
BOT_TOKEN = "1234567890:ABCDEFGHIJKLMNOPQRSTUVWXYZ"  # Reemplaza con tu BOT_TOKEN
...
# Archivo para almacenar permisos de usuarios
PERMISSIONS_FILE = "user_permissions.json"

# ID del administrador
ADMIN_ID = 1742433244

# Estructura predeterminada para el archivo de permisos
DEFAULT_PERMISSIONS = {
    "users": {},
    "maintenance": False
}
...
def load_permissions():
    """Cargar permisos desde archivo o crear predeterminado si no existe"""
...
def is_maintenance_mode():
    ...
    return permissions.get("maintenance", False)

def set_maintenance_mode(status):
    """Establecer estado del modo mantenimiento"""
    permissions = load_permissions()
...
def parse_size(size_str):
    """Convertir cadena de tamaño a bytes"""
...
def add_user_permission(user_id, days, data_limit):
    """Añadir permiso para usuario"""
    permissions = load_permissions()
    
    # Calcular fecha de expiración
    expiration_date = (datetime.now() + timedelta(days=days)).timestamp()
    
    # Convertir límite de datos
    data_limit_bytes = parse_size(data_limit)
    
    # Añadir usuario a permisos
...
def check_user_permission(user_id):
    """Verificar si el usuario tiene permiso para usar el bot"""
    ...
    if is_maintenance_mode():
        return False, "El bot está en mantenimiento. Por favor, intente más tarde."
    
    permissions = load_permissions()
    user_id_str = str(user_id)
    
    # Verificar si el usuario existe en permisos
    if user_id_str not in permissions["users"]:
        return False, "No tienes permiso para usar este bot. Contacta al administrador."
    
    user_data = permissions["users"][user_id_str]
    
    # Verificar si el permiso ha expirado
    if time() > user_data["expiration_date"]:
        return False, "Tu permiso ha expirado. Contacta al administrador para renovarlo."
    
    # Verificar si el usuario ha alcanzado el límite de datos
    if user_data["data_used"] >= user_data["data_limit"]:
        return False, "Has alcanzado tu límite de datos. Contacta al administrador para aumentarlo."
    
    return True, None

def update_data_usage(user_id, bytes_used):
    """Actualizar uso de datos para usuario"""
    if is_admin(user_id):
        return True
    ...
    if user_id_str in permissions["users"]:
        permissions["users"][user_id_str]["data_used"] += bytes_used
        save_permissions(permissions)
        return True
...
def get_user_info(user_id):
    ...
    if user_id_str in permissions["users"]:
        user_data = permissions["users"][user_id_str]
        expiration_date = datetime.fromtimestamp(user_data["expiration_date"]).strftime('%Y-%m-%d')
        data_limit_gb = user_data["data_limit"] / (1024 * 1024 * 1024)
        data_used_gb = user_data["data_used"] / (1024 * 1024 * 1024)
        
        return {
            "expiration_date": expiration_date,
            "data_limit_gb": round(data_limit_gb, 2),
            "data_used_gb": round(data_used_gb, 2),
            "data_remaining_gb": round(data_limit_gb - data_used_gb, 2)
        }
...
async def process_download(client: Client, message: Message, username: str, send):
    ...
    has_permission, error_message = check_user_permission(user_id)
    if not has_permission:
        await send(error_message, quote=True)
        return
    ...
    if message.media:  # Si es un archivo multimedia
        ...
        try:
            ...
            if Path(str(root[username]["actual_root"]) + "/" + filename).stat().st_size == filesize:
                ...
                await update_data_usage(user_id, filesize)
            else:
                file_name = str(root[username]["actual_root"]) + "/" + filename
                unlink(file_name)
...
# Comandos nuevos para el sistema de permisos
@app.on_message(filters.command("permiso") & filters.private)
async def cmd_permiso(client, message):
    user_id = message.from_user.id
    ...
    try:
        ...
        if len(args) != 3:
            await message.reply("Uso correcto: /permiso [user_id] [días] [límite_datos]")
            return
        ...
        # Añadir permiso
        add_user_permission(target_user_id, days, data_limit)
        
        await message.reply(f"Permiso concedido al usuario {target_user_id} por {days} días con un límite de {data_limit}.")
    except Exception as e:
        await message.reply(f"Error al procesar el comando: {str(e)}")

@app.on_message(filters.command("mant") & filters.private)
async def cmd_mant(client, message):
    user_id = message.from_user.id
    ...
    await message.reply("Modo de mantenimiento activado. Solo el administrador puede usar el bot.")
            ...
            if user_id != str(ADMIN_ID):
                try:
                    await client.send_message(
                        int(user_id),
                        "El bot está en mantenimiento. Por favor, intente más tarde."
                    )
                    await asyncio.sleep(0.5)  # Para evitar límites de flood
                except Exception:
                    pass
    except Exception as e:
        await message.reply(f"Error al notificar a los usuarios: {str(e)}")

@app.on_message(filters.command("mantoff") & filters.private)
async def cmd_mantoff(client, message):
    user_id = message.from_user.id
    ...
    await message.reply("Modo de mantenimiento desactivado. Todos los usuarios pueden usar el bot nuevamente.")
        ...
        for user_id in all_users:
            if user_id != str(ADMIN_ID):
                try:
                    await client.send_message(
                        int(user_id),
                        "El bot está nuevamente disponible. Gracias por su paciencia."
                    )
                    await asyncio.sleep(0.5)  # Para evitar límites de flood
                except Exception:
                    pass
...
async def cmd_info(client, message):
    ...
    if user_info:
        await message.reply(
            ...
            f"🗓️ Fecha de expiración: {user_info['expiration_date']}\n"
            f"💾 Límite de datos: {user_info['data_limit_gb']} GB\n"
            f"📈 Datos usados: {user_info['data_used_gb']} GB\n"
            f"📉 Datos restantes: {user_info['data_remaining_gb']} GB"
        )
    else:
        await message.reply("No tienes permisos activos en este bot.")
...
async def handle_message(client, message):
    ...
    # Verificar permiso del usuario (excepto para comandos específicos)
    if message.text and not (message.text.startswith('/start') or 
                            message.text.startswith('/permiso') or 
                            message.text.startswith('/mant') or 
                            message.text.startswith('/mantoff') or 
  ...
  #      await message.reply_text("No tienes acceso a este Bot\nContacta a @Stvz20")
    ...
    if message.text.startswith('/start'):
        ...
        try:
            ...
            # Obtener tamaño del archivo para actualizar uso de datos
            ...
            # Actualizar uso de datos
            await update_data_usage(user_id, file_size)
...
@app.on_callback_query(filters.regex(r"^get_referral_link$"))
